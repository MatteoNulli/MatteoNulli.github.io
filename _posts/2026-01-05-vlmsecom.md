...
date: 2026-08-30 14:24:00
description: E-commerce adaptation of SOTA VLMs preserving general capabilities
tags: Multimodal-Learning Vision-Language-Modelling E-commerce Model-Adaptation VLMs Benchmarking
# categories: Multimodal-Learning
thumbnail: assets/img/ecomvlm5.png
mathjax: true
math: true
---


##### <b>Matteo Nulli</b>, Vladimir Orshulevich, Tala Bazazo, Christian Herold, Michael Kozielski, Marcin Mazur, Szymon Tuzel, Cees G. M. Snoek, Seyyed Hadi Hashemi, Omar Javed, Yannick Versley, Shahram Khadivi
###### eBay, University of Amsterdam 
###### <img src="https://upload.wikimedia.org/wikipedia/commons/1/1b/EBay_logo.svg" alt="eBay" height="24"/> &nbsp; <img src="https://upload.wikimedia.org/wikipedia/commons/d/d1/Amsterdamuniversitylogo.svg" alt="University of Amsterdam" height="24"/> &nbsp;
###### 📄 [Paper]() | 📝 [Blogpost](https://matteonulli.github.io/blog/2025/vlmsecom/)
###### *Accepted to EACL 2026 Industry Track*
<br>
...
## Motivation
If you’ve ever shopped online for something even mildly specific—say, a “navy waterproof trail running jacket with taped seams”—you already know the problem: product understanding isn’t just text. It’s photos. It’s packaging. It’s messy seller-generated descriptions. It’s half-structured attributes that *sometimes* help and *often* don’t. And it’s usually multiple images that each reveal different parts of the truth.

E-commerce product understanding is, by nature, **deeply multimodal**.

Yet most production shopping systems still lean heavily on text matching—titles, short descriptions, maybe a few extracted specs—because it’s reliable, scalable, and easy to index. The issue is that customers don’t shop that way. People compare photos for color, texture, size cues, labels, logos, and packaging details, and then reconcile what they saw with whatever the listing claims.

Meanwhile, the model landscape has changed fast. Large Language Models have started showing strong performance on e-commerce tasks, especially when you tailor them to the domain—either through adaptation strategies or domain-aware training recipes ([1](#llama-3-herd-2024), [2](#scaling-instruction-finetuned-2024), [3](#peng2024ecellmgeneralizinglargelanguage), [4](#herold2025domainadaptationfoundationllms)). But translating that success into the **vision–language** setting is not automatic—and that gap is exactly what this work is about.
...
General-purpose Vision–Language Models (VLMs) have reached a point where multimodal assistants are finally practical. Modern models can jointly reason over images and text, follow instructions, and generalize across tasks, building on strong vision and language backbones ([5](#vit-2021), [6](#attention-is-all-you-need-2017)). Recent families—such as LLaVA and its successors ([7](#visual-instruction-tuning-2023), [9](#llava-next-2024), [10](#llavonevision-2024)), Qwen3-VL ([11](#qwen2-5-vl-2025)), InternVL4 ([12](#internvl2-2024)), and Gemma3 ([13](#gemma-3-2025))—demonstrate how quickly the field is advancing.
However, e-commerce remains a difficult target. Shopping scenarios demand attribute-centric reasoning, aggregation across multiple images, robustness to noisy seller content, and strict instruction following—requirements that go well beyond standard captioning or VQA. While multimodal benchmarks exist ([15](#mme-2024), [16](#mmbench-2024), [17](#evaluating-vlms-right-way-2024)), they only partially capture these shopping-specific challenges.

We study whether strong e-commerce performance truly requires a custom language model, or whether targeted adaptation of vision components is sufficient. Through extensive ablations across different visual and text decoders, combined with efficient adaptation techniques such as low-rank updates ([22](#lora-2021)), we show that careful multimodal adaptation can yield substantial gains.
In parallel, we introduce a new evaluation suite designed around real online retail use-cases. It emphasizes deep product understanding, strict instruction following, and multi-image reasoning—dimensions that are underrepresented in existing benchmarks.

Our main finding is simple: **general-purpose VLMs can be effectively adapted to e-commerce with targeted, reproducible strategies that preserve broad multimodal competence**. This enables meaningful improvements on shopping tasks without sacrificing performance in other domains, and provides a clearer path toward realistic multimodal shopping assistants.
...
<div class="row mt-3">
    ...
    <a id="fig-sit-vs-ours"></a>
        {% include figure.liquid loading="eager" path="./assets/img/verification_ebay.png" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    Figure 3: 
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
    <a id="fig-sit-vs-ours"></a>
        {% include figure.liquid loading="eager" path="./assets/img/ecomvlmfig1.png" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    Figure 3: 
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
    <a id="fig-sit-vs-ours"></a>
        {% include figure.liquid loading="eager" path="./assets/img/ecomvlmfig2.png" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    Figure 4: 
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
    <a id="fig-sit-vs-ours"></a>
        {% include figure.liquid loading="eager" path="./assets/img/ecomvlmfig3.png" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    Figure 5: 
